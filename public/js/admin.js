...
function importRow(item, index) {
  const alerts = [...(item.warnings || []), ...(item.errors || [])];
  ...
  return `
    <tr>
      ...
      <td>${formatNumber(item.meta_meq_total || 0, 2)}</td>
      <td>${segmentLabel}</td>
      <td><small>${escapeHtml(item.source_text || '-')}</small></td>
      <td>${alerts.length ? alerts.map((alert) => `<div>${escapeHtml(alert)}</div>`).join('') : '<span class="status-tag green">Sem alertas</span>'}</td>
      <td>
...
function summaryText(items, sourceFileName) {
  const warnings = items.reduce((total, item) => total + (item.warnings || []).length + (item.errors || []).length, 0);
  return `${items.length} linha(s) lida(s) de ${sourceFileName || 'arquivo'} | ${warnings} alerta(s) para revisar.`;
}
...
export async function initAdminModule() {
  ...
  function renderImportReview() {
    ...
    goalImportSummary.textContent = currentImportItems.length
      ? summaryText(currentImportItems, currentImportFileName)
      : 'Nenhuma linha lida.';
  ...
  document.getElementById('goalImportExcelForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      renderImportReview();
      goalImportFeedback(summaryText(currentImportItems, currentImportFileName), currentImportItems.length ? 'success' : 'error');
    } catch (error) {
  ...
  goalImportTableBody.addEventListener('click', (event) => {
    ...
    renderImportReview();
    goalImportFeedback(summaryText(currentImportItems, currentImportFileName));
  });

  goalImportSaveButton.addEventListener('click', async () => {
    if (!currentImportItems.length) {
      goalImportFeedback('Nenhuma linha pronta para salvar.', 'error');
      return;
    }

    try {
      goalImportFeedback('Salvando metas confirmadas...');
      const response = await api.confirmGoalImport(currentImportItems);
      resetImportReview(`${response.items?.length || currentImportItems.length} meta(s) salva(s) com sucesso.`, 'success');
      await refreshAdmin();
    } catch (error) {
